...
export class AzureStreamableMCPServer {
  ...
  createServer(): McpServer {
    ...
    // Register get_contacts tool
    server.registerTool(
      'get_contacts',
      {
        title: 'Get Contacts',
        description: 'Get contacts from Outlook/Exchange',
        inputSchema: {
          limit: z
            .number()
            .optional()
            .describe('Number of contacts to retrieve (default: 10)'),
          search: z.string().optional().describe('Search query for contacts'),
        },
      },
      async ({ limit = 10, search }) => {
        ...
        const contactsResult = await this.azureClient.getContacts({
          limit,
          search,
        });
        ...
        if (contactsResult.success && contactsResult.data) {
          ...
          contactsResult.data.forEach(contact => {
            content += `${contact.id},"${contact.displayName.replace(/"/g, '""')}",${contact.userPrincipalName},${contact.mail || ''},${contact.jobTitle || ''},${contact.department || ''},${contact.officeLocation || ''}\n`;
          });
    ...
    server.registerTool(
      'send_message',
      {
        ...
        description: 'Send an email message via Outlook/Exchange',
        inputSchema: {
          to: z
            .array(z.string())
      ...
      async ({ to, subject, body, importance = 'normal' }) => {
        if (!this.azureClient) {
          throw new Error('Azure client not initialized');
        }

        const sendResult = await this.azureClient.sendMessage({
          to,
          subject,
          body,
          importance,
        ...
        if (sendResult.success && sendResult.data) {
          return {
            content: [
              {
                type: 'text',
                text: `Message sent successfully. ID: ${sendResult.data.id}`,
              },
            ],
          };
        } else {
